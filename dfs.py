def build_graph(edges):
    ...
    Граф представляется в виде словаря: ключ - вершина, значение - список смежных вершин
    
    Аргументы:
    edges -- список рёбер в виде кортежей (u, v)
    ...
    for u, v in edges:
        ...
        if v not in graph:
            graph[v] = []
        # Добавляем ребро от u к v
        graph[u].append(v)
    return graph

def dfs(graph, start):
    """
    Обход графа в глубину, начиная с вершины start
